...
public class IceDrinkMenuTab extends JPanel implements ActionListener{
	JTable table;
	JPanel menu_panel,content,rightpanel;
	TextField name_field,price_field;
	JButton addbtn,rmbtn;
	JScrollPane scrolPane;
	public IceDrinkMenuTab() {
		setLayout(new BorderLayout());
		
		
		String header[]= {"메뉴","가격"};		
		menu_panel=new showMenu(header, Admin_Kiosk_App.menuList.getIceDrinkmenuToStringMap());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource()==addbtn) {
			Admin_Kiosk_App.menuList.IceDrinkList.put(name_field.getText(), new Product(name_field.getText(),Integer.parseInt(price_field.getText())));
			update_table();			
			Product_menu_list_controller_ver_GUI.save("menu.obj");

		}else if (e.getSource()==rmbtn) {
			Admin_Kiosk_App.menuList.IceDrinkList.remove(name_field.getText());
			update_table();		
			Product_menu_list_controller_ver_GUI.save("menu.obj");
